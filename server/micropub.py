...
def micropub_endpoint():
    if request.method == "POST":
        ...
        if (
            ...
            and not request.headers["Content-Type"].startswith(
            ...
            return (
                jsonify(
                    {
                        "message": "Please send your request with the Content-Type application/x-www-form-urlencoded, \
        ...
        if request.headers["Content-Type"].startswith("application/json"):
            ...
            if request.files.getlist("photo[]"):
                ...
                for photo in request.files.getlist("photo[]"):
                    photo.save(
                        os.path.join(UPLOAD_FOLDER, secure_filename(photo.filename))
                    )
                        ...
                        "file": (
                            ...
                            open(
                                os.path.join(
                                    UPLOAD_FOLDER,
                                    secure_filename(
                                        request.files.get("photo").filename
                                    ),
                                ),
                                "rb",
                            ),
        ...
        if (
            ...
            and object_type["properties"].get("content")
            and type(object_type["properties"].get("content")[0]) == dict
        ):
            ...
            object_type.get("properties")
            and object_type["properties"].get("content")
            and type(object_type["properties"].get("content")[0]) == str
        ):
        ...
        for root_property in root_properties:
            if (
                object_type.get(root_property)
                and not object_type.get("drank")
                and object_type[root_property]["properties"].get("content")
                and type(object_type[root_property]["properties"].get("content")[0])
                == str
            ):
        ...
        if object_type.get("properties") and object_type.get("properties").get(
            "checkin"
        ):
        ...
        for item in object_types:
            ...
            if object_type.get(property_value) or object_type["properties"].get(
                property_value
            ):
                return create_items.process_social(
                    repo, front_matter, interactions.get(interaction_name), content
                )
        ...
        if post_type == "entry":
            return create_items.process_social(
                repo, front_matter, interactions.get("note"), content
            )
    ...
    if (
        ...
        try:
            ...
            yaml_to_json = yaml.load(
                "".join([c for c in content[:end_of_yaml] if "---" not in c]),
    ...
    if request.method == "POST":
        if not request.headers["Content-Type"].startswith("multipart/form-data"):
            return (
                jsonify(
                    {
                        "message": "Please send your request with the Content-Type multipart/form-data."
                    }
                ),
                400,
            )
